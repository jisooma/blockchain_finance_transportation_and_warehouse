...
contract contractHash {
    ...
    function getContarctHash(uint id,string type1)public view returns(uint,uint,string ,string ,string,string,string){
        for(uint i=1;i<=idLists.length;i++){
            if(contractHashes[i].id==id && (keccak256(abi.encodePacked(contractHashes[i].Type)) == keccak256(abi.encodePacked(type1)))){
                ...
                return (no,ID,partyA,partyB,Type,hash,updateTime);
            }
        }
    }
}
